...
import {
  ...
  RecyclingTrendCard,
  RecentWasteListCard,
} from "./components/WasteDataCards_New";
...
export default function App() {
  ...
  const [showSignupDialog, setShowSignupDialog] =
    useState(false);
  ...
  const [showServicesPage, setShowServicesPage] =
    useState(false);
  ...
  const [showWorkProgress, setShowWorkProgress] =
    useState(false);
  const [showQualityInspection, setShowQualityInspection] =
    useState(false);
  const [showEquipmentStatus, setShowEquipmentStatus] =
    useState(false);
  const [
    showProductionPerformance,
    setShowProductionPerformance,
  ] = useState(false);
    ...
    <>
      ...
      <AnimatePresence mode="wait">
        ...
        {!showPurchase &&
          ...
          !showMainPage &&
          showStaffAuth && (
            <motion.div key="staff-auth" {...pageTransition}>
              <StaffAuth
                onBack={() => {
          ...
          showStaffLayout && (
            <div className="flex h-screen bg-gray-50">
              ...
              <div className="hidden lg:block">
                <Sidebar
                  currentPage={
                    currentStaffPage === "dashboard"
                      ? "대시보드"
                      : currentStaffPage === "inventory"
                        ? "재고 관리"
                        : currentStaffPage === "shipping"
                          ? "출고 관리"
                          : currentStaffPage === "production"
                            ? "생산 관리"
                            : currentStaffPage === "report"
                              ? "통계 리포트"
                              : currentStaffPage ===
                                  "attendance-stats"
                                ? "출근 통계"
                  ...
                  onInventoryClick={() =>
                    setCurrentStaffPage("inventory")
                  }
                  onShippingClick={() =>
                    setCurrentStaffPage("shipping")
                  }
                  onProductionClick={() =>
                    setCurrentStaffPage("production")
                  }
                  onReportClick={() =>
                    setCurrentStaffPage("report")
                  }
                  onAttendanceStatsClick={() =>
                    setCurrentStaffPage("attendance-stats")
                  }
                  onStaffManagementClick={() =>
                    setCurrentStaffPage("staff-management")
                  }
                  onAuditLogClick={() =>
                    setCurrentStaffPage("audit-log")
                  }
                  onBackupClick={() =>
                    setCurrentStaffPage("backup")
                  }
                  onNotificationClick={() =>
                    setCurrentStaffPage("notification")
                  }
                />
              </div>
